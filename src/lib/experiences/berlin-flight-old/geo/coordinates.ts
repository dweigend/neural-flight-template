...
export function geographicToLocalMeters(
  ...
  if (!isFiniteGeographicCoordinate(reference.origin))
    return zeroLocalCoordinate();
  ...
  const eastMeters =
    deltaLonRadians * Math.cos(originLatRadians) * EARTH_RADIUS_METERS;
...
export function localMetersToGeographic(
  ...
  if (!isFiniteGeographicCoordinate(reference.origin))
    return zeroGeographicCoordinate();
...
export function geographicToEcef(coordinate: GeographicCoordinate): {
  x: number;
  y: number;
  z: number;
} {
  const lat = degreesToRadians(coordinate.latitudeDegrees);
  const lon = degreesToRadians(coordinate.longitudeDegrees);
  const h = coordinate.heightMeters;
...
export function getBerlinMitteEcef(): { x: number; y: number; z: number } {
  return geographicToEcef(BERLIN_MITTE_ORIGIN);
}
