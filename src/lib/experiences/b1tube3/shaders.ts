...
void main() {
  ...
  // Gentle bend so the tube snakes
...
void main() {
  ...
  // Dark pink walls
  col *= 0.35;
  col += vec3(0.8, 0.1, 0.25) * 0.2;

  // Peristalsis pulse visible on walls
  col += vec3(1.0, 0.2, 0.4) * vPeri * 0.4;

  // Subtle light smudge that travels along the tube
  float dz = vZ - uLightZ;
  float lightGlow = exp(-dz * dz * 0.008) * 0.2;
  col += vec3(1.0, 0.5, 0.2) * lightGlow;

  // Thick nebula mist on tube walls (no separate particles — avoids VR tube-in-tube)
  ...
  // Depth haze — thicker nebula feeling far away
