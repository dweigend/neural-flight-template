  ...
  material.onBeforeCompile = (shader: ShaderCompileParameters, renderer) => {
    ...
    shader.fragmentShader = shader.fragmentShader
      ...
      .replace(
        ...
        "// ponytail: keep the cone mask blend but let the base material lighting handle shading; this drops the per-fragment derivative work.\nfloat berlinConeMask = clamp(vBerlinConeMask, 0.0, 1.0);\n#include <map_fragment>\ndiffuseColor.rgb = mix(uBerlinNeutralColor, diffuseColor.rgb, berlinConeMask);\ndiffuseColor.a = mix(uBerlinOutsideOpacity, 1.0, berlinConeMask);",
      );
  };
  material.customProgramCacheKey = () =>
    `${previousProgramCacheKey?.() ?? material.type}:berlin-cone-mask-v4`;
