...
void main() {
  ...
  float pulseMix = pulse1 * pulse2 * pulse3;
  ...
  float waveColor1 = sin(p.z * 0.3 + n1 * 8.0 + uTime * 2.5) * 0.5 + 0.5;
  float waveColor2 = cos(p.x * 0.4 + n2 * 7.0 + uTime * 2.0) * 0.5 + 0.5;
  float colorMix = waveColor1 * waveColor2;
  vec3 colorVariation = mix(mix(uGlowA, uGlowB, colorMix), uAccent, colorMix * 0.4);

  float fresnel = pow(1.0 - abs(dot(vNormal, vec3(0.0, 0.0, 1.0))), 1.2);

  vec3 color = mix(uBase, colorVariation, glow * uGlowIntensity);
  color = mix(color, colorVariation * 1.4, fresnel * 0.5);
  color *= 0.3 + pulseMix * 0.75;
