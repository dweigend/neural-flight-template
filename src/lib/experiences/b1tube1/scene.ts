...
const PHASES: PhaseDef[] = [
	...
	{
		speed: 3, periStrength: 4.0, pushAmp: 2, touchAmp: 0.02, shapeDist: -200, shapeScale: 0.3,
		particleBright: 0.7,
	...
	{
		speed: 5, periStrength: 3.0, pushAmp: 5, touchAmp: 0.03, shapeDist: -180, shapeScale: 0.4,
		particleBright: 0.9,
	...
	{
		speed: 8, periStrength: 2.0, pushAmp: 7, touchAmp: 0.03, shapeDist: -160, shapeScale: 0.5,
		particleBright: 1.0,
	...
	{
		speed: 14, periStrength: 2.5, pushAmp: 12, touchAmp: 0.04, shapeDist: -140, shapeScale: 0.6,
		particleBright: 1.2,
	...
	{
		speed: 18, periStrength: 1.0, pushAmp: 10, touchAmp: 0.02, shapeDist: -80, shapeScale: 1.0,
		particleBright: 1.5,
...
// ── Fog patches ──
function createFogPatches(): { group: THREE.Group; mats: THREE.ShaderMaterial[] } {
	const group = new THREE.Group();
	const mats: THREE.ShaderMaterial[] = [];
	...
	for (let i = 0; i < FOG_PATCH_COUNT; i++) {
		const mat = new THREE.ShaderMaterial({
			uniforms: {
				...
				uColor: { value: new THREE.Color(0.25, 0.02, 0.06) },
				uAlpha: { value: 1.0 },
			},
			vertexShader: `
				varying vec2 vUv;
			...
			fragmentShader: `
				...
				varying vec2 vUv;
				void main() {
					vec2 uv = vUv + uOffset * 0.01;
					float d = distance(uv, vec2(0.5)) * 2.0;
					float a = exp(-d * d * 6.0) * 0.15 * uAlpha;
					a *= 0.6 + 0.4 * sin(uTime * 0.1 + uOffset.x * 5.0);
					if (a < 0.01) discard;
		...
		const angle = Math.random() * Math.PI * 2;
		const radius = Math.random() * TUBE_RADIUS * 0.85;
		const z = 5 - Math.random() * (TUBE_LENGTH + 20);
...
export async function setup(ctx: SetupContext): Promise<B1LesserState> {
	...
	const { group: fogGroup, mats: fogMats } = createFogPatches();
...
function applyPhase(s: B1LesserState, phase: number): void {
	...
	for (const m of o.fogMats) {
		m.uniforms.uColor.value.setRGB(p.fogColor[0], p.fogColor[1], p.fogColor[2]);
	}
}
...
export function tick(
	...
	// Touch oscillation
	s.touchPhase += ctx.delta;
	const touchAmp = p.touchAmp;
	const tilt = Math.sin(s.touchPhase * 1.2) * touchAmp * 0.6;
	const drop = Math.sin(s.touchPhase * 0.4) * touchAmp * 0.4;
	const vibrate = Math.sin(s.touchPhase * 30) * touchAmp * 0.1;
	s.camera.rotation.x = tilt + vibrate;
	s.camera.position.y = drop;

	// Push: scroll tubeGroup instead of moving camera (infinite fly-through)
...
export function dispose(state: ExperienceState, _scene: THREE.Scene): void {
	...
	for (const m of o.fogMats) m.dispose();
	for (const child of o.fogGroup.children) {
