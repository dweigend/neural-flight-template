...
export function startM5Bridge(
  ...
  server.on("connection", (socket: WebSocket) => {
    ...
    socket.on("message", (raw: RawData) => {
      const rawMessage = rawDataToString(raw);
      try {
        const message = parseM5DeviceMessage(rawMessage);
        ...
        console.warn(
          "[m5-bridge] Invalid frame dropped",
          message,
          truncateForLog(rawMessage),
        );
      }
    });
...
function truncateForLog(message: string, maximumLength = 400): string {
...
function mapM5OrientationToControllerMessage(
  ...
  const mappedRoll =
    mapM5Roll(message.pitch, message.roll) *
    m5BridgeRuntimeConfig.rollScale *
